...
$getPrescriptionid=mysql_query("SELECT 1+MAX(prescriptionid) FROM prescription");
$presId=mysql_fetch_array($getPresid);
		if($prescriptionid[0]=='')
		{$prescriptionid=999; }
		else{$prescriptionid=$prescriptionid[0];}
	
	$sqlP=mysql_query("INSERT INTO prescription(prescriptionid,customername,age,gender,address,invoiceid,phone)
				VALUES('$prescriptionid','$customername','$age','$gender','$address','$invoiceid','$phone')  ");
		$sqlI=mysql_query("INSERT INTO invoice(invoiceid, customername,servedby,status,date)
				VALUES('$invoiceid','$customername','$servedby','$status',$'date') ");
